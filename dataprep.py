...
if __name__ == "__main__":
    ...
    paths["ticker"] = paths["BBO_file"].apply(
        lambda x: re.match(".*\/bbo\/(.*)\..*\/.*", x).groups()[0]
    )
    paths["date"] = paths.apply(
        lambda x: re.match(f".*\/(.*)-{x['ticker']}.*", x["BBO_file"]).groups()[0],
        axis=1,
    )
    ...
    for ticker in paths["ticker"].unique():
        if ticker not in ["HD", "RTX", "WBA"]:
            ...
            all_promises = paths[paths["ticker"] == ticker].apply(
                lambda x: dataprep_utils.load_merge_trade_bbo(
                    x["ticker"], x["date"], x["BBO_file"], x["TRADE_file"]
                ),
                axis=1,
            )
